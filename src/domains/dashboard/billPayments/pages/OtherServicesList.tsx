...
import { otherServices } from '../utils/data';
...
const OtherServicesList = () => {
    ...
    const filteredItems = otherServices.filter(item => {
        const serviceName = item.title;
        return checkSubServiceAccessCorporate('Bill Payments', serviceName as string);
    });
    const filteredotherServices = filteredItems.filter(item =>
        identityServices.includes(item.accessKey)
    );
    return (
        <Flex vertical gap={40} className="mt-10">
            {filteredotherServices.length > 0 && (
                <Text className="font-medium text-lg sm:text-xl ">Other Services</Text>
            )}
            <Row gutter={[20, 30]}>
                {filteredotherServices.map((item: TelecomItem, index) =>
                    screens.xs ? (
                        <IconCardMobile
                            icon={item.icon}
                            title={item.title}
