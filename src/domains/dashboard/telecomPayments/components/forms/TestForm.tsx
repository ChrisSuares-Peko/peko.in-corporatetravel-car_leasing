...
import useServiceProviderApi from '../../hooks/useServiceProviderApiTest';
...
const PostpaidForm: React.FC = () => {
    ...
    const { handleTestPay } = usePayment();
    ...
    const initialValues = useMemo(() => {
        ...
        return values;
    }, [selectedBillerId, selectedBillerData]);

    return (
        <Formik
            initialValues={initialValues}
            onSubmit={async values => {
                await handleTestPay(values, serviceProviderData?.[0].label);
            }}
